...
happyExpList = Happy_Data_Array.listArray (0,26) ([16,32768,0,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Entry","true","false","if","else","'('","')'","'{'","'}'","NUM","VAR","';'","'&&'","'||'","'*'","'-'","'+'","'='","'<='","'<'","'/='","'=='","'int'","%eof"]
        bit_start = st * 27
        bit_end = (st + 1) * 27
...
happyNewToken action sts stk (tk:tks) =
	...
	OBRACE -> cont 11;
	CBRACE -> cont 12;
	NUM happy_dollar_dollar -> cont 13;
	VAR happy_dollar_dollar -> cont 14;
	EOL -> cont 15;
	AND -> cont 16;
	OR -> cont 17;
	MUL -> cont 18;
